...
import type { Direction, ItemIdentifier } from "../items";
import type { ElementPosition, NodeMeta } from "../nodes";
...
export const getDropLinePosition = <T extends ItemIdentifier>(
  ...
  const dropLineDirection = getDropLineDirection(
    nodeMeta.width,
    nodeMeta.height,
    [x, y],
    direction,
...
export const checkIsInStackableArea = <T extends ItemIdentifier>(
  ...
  if (direciton === "vertical") {
    ...
    const isInStackableAreaY =
      nodeTop + stackableAreaThreshold <= y &&
      y <= nodeBottom - stackableAreaThreshold;
  ...
  if (direciton === "horizontal") {
    ...
    const isInStackableAreaX =
      nodeLeft + stackableAreaThreshold <= x &&
      x <= nodeRight - stackableAreaThreshold;
