...
export const Item = <T extends ItemIdentifier>(props: Props<T>) => {
  ...
  const ancestorIdentifiers = [
    ...groupContext.ancestorIdentifiers,
    props.identifier,
  ];
  ...
  const isLonelyBottomOrRight =
    (isLonely || props.isLonelyBottomOrRight) ?? false;
  const isUsedCustomDragHandlers = props.isUsedCustomDragHandlers ?? false;

  const useGestureConfig: UseGestureConfig = {
    ...(props.useGestureConfig || {}),
  };
  if (props.onTap) {
  ...
  const onDrag = React.useCallback(
    (isDown: boolean, absoluteXY: [number, number]) => {
      if (!isDown) return;
    ...
    listContext.onDragEnd({
      ...
      nextGroupIdentifier:
        destinationMeta !== undefined
          ? destinationMeta.groupIdentifier
          : groupContext.identifier,
      nextIndex:
        destinationMeta !== undefined ? destinationMeta.index : props.index,
      listIdentifier: listContext.listIdentifier,
    });
  }, [
    listContext.onDragEnd,
  ...
  const onMoveForItems = React.useCallback(
    (
      draggingNodeMeta: NodeMeta<T>,
      hoveredNodeMeta: NodeMeta<T>,
      ...
      const disableTopOrLeft =
        ...
        ((listContext.direction === "horizontal" &&
          absoluteXY[0] <
            hoveredNodeMeta.absolutePosition.left +
              hoveredNodeMeta.width / 2) ||
          (listContext.direction === "vertical" &&
            absoluteXY[1] <
              hoveredNodeMeta.absolutePosition.top +
                hoveredNodeMeta.height / 2));
      ...
      const dropLineElement =
        listContext.dropLineElementRef.current ?? undefined;
      setDropLineElementStyle(
        dropLineElement,
        absoluteXY,
        hoveredNodeMeta,
      ...
      const dropLineDirection = getDropLineDirectionFromXY(
        absoluteXY,
        hoveredNodeMeta,
        listContext.direction,
      );
      ...
      const isComeFromStackedGroup =
        destinationMeta !== undefined &&
        destinationMeta.groupIdentifier !== undefined &&
        destinationMeta.index === undefined;
    ...
    clearingDraggingNodeTimeoutIdRef.current = window.setTimeout(() => {
      ...
      if (hoveredMeta) {
        const isDifferentList =
          hoveredMeta.listIdentifier !== listContext.listIdentifier;

        // if it is the same list but different identifier, we don't reset the visual feedback.
        if (!isDifferentList && hoveredMeta.identifier !== props.identifier)
          return;
      }
    ...
    onMove: ({ xy }) => {
      ...
      if (hoveredMeta) {
        ...
        if (isDifferentList) {
          ...
          const hoveredNodeAncestors =
            listContext.hoveredNodeMetaRef.current?.ancestorIdentifiers ?? [];

          if (
            hasItems &&
            checkIsAncestorItem(props.identifier, hoveredNodeAncestors)
          )
            return;
        }
      }
      ...
      if (
        checkIsAncestorItem(
          listContext.draggingNodeMeta.identifier,
          ancestorIdentifiers,
        )
      )
        return;
  ...
  const dragHandlers: React.ContextType<typeof ItemContext>["dragHandlers"] = {
    onDragStart,
    onDrag,
    onDragEnd,
  };
  ...
  const contentElement = React.useMemo((): JSX.Element => {
    ...
    const isDragging =
      draggingNodeMeta !== undefined &&
      props.identifier === draggingNodeMeta.identifier;
    const { renderPlaceholder, renderStackedGroup, itemSpacing, direction } =
      listContext;

    const rendererMeta:
      | Omit<PlaceholderRendererMeta<any>, "isGroup">
      | StackedGroupRendererMeta<any> = {
      identifier: props.identifier,
      groupIdentifier: groupContext.identifier,
      listIdentifier: listContext.listIdentifier,
      index: props.index,
    ...
    if (isDragging && renderPlaceholder !== undefined) {
      const style = getPlaceholderElementStyle(
        draggingNodeMeta,
        itemSpacing,
        direction,
      );
    ...
    if (
      listContext.stackedGroupIdentifier === props.identifier &&
      renderStackedGroup !== undefined
    ) {
      const style = getStackedGroupElementStyle(
        listContext.hoveredNodeMetaRef.current,
        itemSpacing,
        direction,
      );
    ...
    return (
      <div
        ...
        data-column-direction={props.direction}
  ...
  if (!isGroup)
    return (
      <ItemContext.Provider value={{ isLocked, dragHandlers }}>
        {contentElement}
